...
if navigation_page == "📊 Historical Overview":
    ...
    with kpi_col1:
        st.markdown(f"<div class='metric-container'><div class='metric-title'>Total Revenue Volume</div><div class='metric-value'>${total_revenue_accrued:,.2f}</div></div>", unsafe_allow_html=True)
    with kpi_col2:
        st.markdown(f"<div class='metric-container'><div class='metric-title'>Average Lead Time</div><div class='metric-value'>{mean_shipping_lead_time:.2f} Days</div></div>", unsafe_allow_html=True)
    with kpi_col3:
        st.markdown(f"<div class='metric-container'><div class='metric-title'>Processed Transactions</div><div class='metric-value'>{total_processed_orders:,} Units</div></div>", unsafe_allow_html=True)
    ...
    predictive_chart_fig.update_layout(
        ...
        yaxis_title="Calculated Value Stream ($)",
        template="plotly_white", 
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1) # <-- Fixed Plotly Legend syntax here
    )
